...
class PipelineReadModelTest(unittest.TestCase):
    ...
    def test_queue_show_exposes_classify_embedding_batch_domain_view_details(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["domain_view"]["parent_stage"], "clustered_event_extraction")
            self.assertEqual(result["domain_view"]["task_group_id"], "group-1")
            self.assertEqual(result["domain_view"]["queue_task_type"], "classify.embedding")
            self.assertEqual(result["domain_view"]["concurrency_key"], "classify.embedding")
            self.assertEqual(result["domain_view"]["max_concurrency"], 2)
            ...
            self.assertIsNone(result["domain_view"]["batch_index"])
            self.assertIsNone(result["domain_view"]["batch_count"])
            self.assertEqual(result["domain_view"]["item_index"], 0)
            self.assertEqual(result["domain_view"]["item_count"], 2)
    ...
    def test_queue_show_exposes_classify_relevance_batch_domain_view_details(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["domain_view"]["parent_stage"], "clustered_event_extraction")
            self.assertEqual(result["domain_view"]["queue_task_type"], "classify.batch_relevance")
            self.assertEqual(result["domain_view"]["concurrency_key"], "classify.llm")
            self.assertEqual(result["domain_view"]["max_concurrency"], 2)
            self.assertEqual(result["domain_view"]["batch_index"], 1)
            self.assertEqual(result["domain_view"]["batch_count"], 2)
            self.assertEqual(result["domain_view"]["item_index"], 0)
            self.assertEqual(result["domain_view"]["item_count"], 2)
    ...
    def test_queue_show_exposes_clustered_extraction_llm_batch_domain_view_details(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["domain_view"]["parent_stage"], "clustered_event_extraction")
            self.assertEqual(result["domain_view"]["queue_task_type"], "classify.clustered_event_extraction.batch")
            self.assertEqual(result["domain_view"]["concurrency_key"], "classify.llm")
            self.assertEqual(result["domain_view"]["max_concurrency"], 3)
            self.assertEqual(result["domain_view"]["batch_index"], 1)
            self.assertEqual(result["domain_view"]["batch_count"], 3)
            self.assertEqual(result["domain_view"]["item_index"], 0)
            self.assertEqual(result["domain_view"]["item_count"], 3)
    ...
    def test_queue_show_exposes_clustered_merge_llm_batch_domain_view_details(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["domain_view"]["parent_stage"], "clustered_event_merge")
            self.assertEqual(result["domain_view"]["queue_task_type"], "classify.clustered_event_merge.batch")
            self.assertEqual(result["domain_view"]["concurrency_key"], "classify.llm")
            self.assertEqual(result["domain_view"]["max_concurrency"], 3)
            self.assertEqual(result["domain_view"]["batch_index"], 2)
            self.assertEqual(result["domain_view"]["batch_count"], 4)
            self.assertEqual(result["domain_view"]["item_index"], 1)
            self.assertEqual(result["domain_view"]["item_count"], 4)
