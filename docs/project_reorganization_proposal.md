...
- report 数据模型、utils、规则表、IO helper 和分类/摘要/核验 stages 已迁入 `modnews/service/report/`；`src.models`、`src.utils.*`、`src.rules.*`、`src.io.*` 与 `src.pipeline.classifier|summarizer|verifier` 现在是兼容转发层，report 子模块已改用新路径。
