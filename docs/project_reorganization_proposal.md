...
- report 数据模型、utils 和规则表已迁入 `modnews/service/report/`；`src.models`、`src.utils.*`、`src.rules.*` 现在是兼容转发层，report 子模块已改用新路径。
...
- report 层已有 `modnews/service/report` facade、新 CLI 入口、主 pipeline 实现、模型、utils 和规则表；IO、评分/摘要/核验子模块仍暂时依赖 `src/` 命名空间。
