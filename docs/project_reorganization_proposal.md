...
- 默认 pipeline run 已改成多个独立注册 step 组装：`IngestPipelineStep` 负责注册 RSS/NewsNow/managed web source 任务，`CombineIngestPipelineStep` 负责收口 ingest 输出，`ClassifyPipelineStep` 负责注册 clustered extraction/merge，`ReportPipelineStep` 负责注册统一 `report.generate` 任务；任务依赖由 `EventQueue` 推进。公开 `run start` CLI/API 不再提供旧同步端到端 runner fallback。`PipelineManager.on_task_blocked` 会把 blocked 回调转发给已注册 step，step 可据此安全跳过可选任务或注册替代任务。
...
- report 生成主实现已迁入 `modnews/service/report/pipeline.py`，并接入 `modnews report generate`、`modnews-report` 和 `newsagent-report` 入口；同时新增统一队列 task `report.generate`，默认 pipeline run 会在 classify merge 成功后自动注册并执行 report。源码内 `src/` report 兼容转发层已删除。
...
- report 层已有 `modnews/service/report` facade、新 CLI 入口、主 pipeline 实现、统一 `report.generate` task、配置默认值、模型、utils、规则表、IO helper、evidence、editor、reporter 和 stages；wheel 不再发布 `src` 包，源码内 `src/` report 兼容转发层已删除。
