...
- classify 内部 batch/embedding 并发已集中到 `modnews/service/classify/batch_executor.py`；executor 现在带有 `task_type`、`concurrency_key`、batch index 和 labels 元数据，并定义了可替换 backend 协议。当前默认 backend 仍是本地线程池，后续可把该 backend 替换为 `TaskEvent` 注册，而不是让各 step 自己维护线程池。
