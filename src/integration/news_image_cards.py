...
def main() -> None:
    ...
    parser.add_argument("--limit", type=int, default=30, help="Maximum number of report items to render.")
        ...
        manifest_items.append(
            {
                ...
                "section_title": _section_title(item.section),
                "title": item.title,
                "brief": item.brief,
                "why": item.why,
...
def _section_title(section: str) -> str:
    meta = SECTION_META.get(section, SECTION_META["watchlist"])
    return f"{meta['layer']}：{meta['label']}"
